...
public class KafkaStreamProcessor
{
    ...
    public KStream<String, Long> kStream(StreamsBuilder kStreamBuilder)
    {
        ...
        Predicate<String, Long> isDivisibkleBy10 = (k, v) -> v == 10;
        Predicate<String, Long> isDivisibleBy5 = (k, v) -> (v % 5) == 0;
        KStream<String, Long>[] branches = stream.branch(isDivisibkleBy10, isDivisibleBy5);

        branches[0].mapValues(v -> v * 2).to("double-output");
        branches[1].mapValues(v -> v * v).to("square-output");

        Arrays.stream(branches)
                .forEach(b -> b.to("new-stream"));
